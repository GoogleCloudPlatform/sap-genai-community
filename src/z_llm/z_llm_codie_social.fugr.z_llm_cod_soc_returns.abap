FUNCTION z_llm_cod_soc_returns.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
* LLM should have identified the order number
      ...
      IF lv_hybris_order IS INITIAL.

* This just simply could be the Store purchase. The AI should detect if this was a store or online purchase and proceed accordingly to instructions.
        DATA(lcl_llm_no_order) = NEW zcl_cs_hub_llm_util( ).

        ev_function_response = lcl_llm_no_order->llm_check_query_purchase_type( query = lv_original_request ).

      ELSE.


        DATA(lv_bstnk) = zcl_cs_hub_llm_util=>check_hybris_order( lv_hybris_order ).
...
* Order number found.
          ...
          "ev_function_response = lcl_llm->llm_get_returns_data( lv_item_descryption ).
