...
* Call templated methods.
  ...
  DATA(freshdesk_ticket_id) = lcl_template_util->get_memory_id( memory_id = 'ZLLM_CODIE_SYSTEM_KEY' ).
...
*     With the app id and business function now set, we should re-instantiate the template class.
      ...
      IF it_function_parameters IS NOT INITIAL.
        ...
        DATA : ls_agent_param LIKE LINE OF lt_agent_params.

        ls_agent_param-param_name = 'UUIDKEY'.
        ls_agent_param-param_value = lv_uuid.

        APPEND ls_agent_param TO lt_agent_params.
        CLEAR : ls_agent_param.

*       Since Freshdesk ID is part of the function call parameters, but its value is read from the memory,
*       the below check is done to replace the existing entry, if it exists with the correct FD ticket ID
        IF line_exists( lt_agent_params[ param_name = 'FRESHDESKTICKET' ] ).
          DELETE lt_agent_params WHERE param_name = 'FRESHDESKTICKET'.
        ENDIF.

        ls_agent_param-param_name = 'FRESHDESKTICKET'.
        ls_agent_param-param_value = freshdesk_ticket_id.
...
*
          cv_prompt = |{ cv_prompt }\n| &
                      |Respond with TRUE/FALSE based on whether action has been completed|.
