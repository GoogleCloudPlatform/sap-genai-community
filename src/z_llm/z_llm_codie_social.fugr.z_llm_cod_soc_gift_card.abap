FUNCTION z_llm_cod_soc_gift_card.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*       Check if the hybris order number provided exists in the giftcards table
*       Doing this to avoid the next step of going into the big method that gets all the data, in case this select fails
      SELECT SINGLE order_number
        FROM zsavvy_giftcards
        INTO @DATA(lv_order_exists)
       WHERE order_number = @hybris_order.
...
* Hybris order exists in savvy table, not vbak. So if ev_response is blank, then nothing exists.
