FUNCTION z_llm_cod_soc_ord_missing_ret.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
* The LLM may have identified the order but it might be formatted with spaces. Remove.
        ...
        CATCH cx_sy_itab_line_not_found.
          hybris_order = |UNKNOWN|.
      ENDTRY.


      IF hybris_order IS INITIAL OR hybris_order = 'UNKNOWN'.

* Set the intro.
        ls_prompt = lcl_llm_util->fetch_prompt( iv_prompt_key = 'INTROCONTEXT' ).
        cv_prompt = |{ lv_original_request } \n\n { ls_prompt-prompt_text }|.


* Set the prompt.
        ls_prompt = lcl_llm_util->fetch_prompt( iv_prompt_key = 'ORDERNOORDERCONTEXT' ).
        cv_prompt = |{ cv_prompt }'\n\n'{ ls_prompt-prompt_text } |.
...
* Update the global context to error context.
          ls_prompt = lcl_llm_util->fetch_prompt( iv_prompt_key = 'ORDERNOTFOUNDCONTEXT' ).
          cv_prompt = |{ cv_prompt } \n\n { ls_prompt-prompt_text }|.
