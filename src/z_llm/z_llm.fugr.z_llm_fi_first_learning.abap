FUNCTION z_llm_fi_first_learning.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"----------------------------------------------------------------------
  DATA : function_call_uuid TYPE guid16,
         app_id             TYPE zllm_app_id VALUE 'FI',
         business_function  TYPE zllm_buss_function_name VALUE 'AP_INVOICE',
         lv_memory_id       TYPE char50.
...
* Get the ABAP stack.
  ...
  TRY.
      ...
      data(ls_prompt) = lcl_llm_util->fetch_prompt( iv_prompt_key = 'SYSTEM' ).
