FUNCTION z_llm_cod_soc_sup_ord_status.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
**********************************************************************
* Content Analyser - for text and images
* For images - image URL needs to be extracted by the agent and passed to the logic below
**********************************************************************
  TRY.

      DATA(lref_llm_content_analyser) = NEW zcl_llm_content_analyser_util(
        iv_requester_app_id        = CONV zllm_app_id( app_id )
        iv_requester_buss_function = CONV zllm_buss_function_name( business_function ) ).


      DATA(lv_content_analysis) = lref_llm_content_analyser->analyse_content( it_function_parameters = it_function_parameters
                                                                              iv_uuid_key            = CONV guid16( lv_uuid )
*                                                                             iv_content             =
                                                                            ).

*   We do not want to terminate the process if something goes wrong here. So we continue.
    CATCH zcx_llm INTO DATA(lcx_content_analysis).
  ENDTRY.
