FUNCTION Z_LLM_SUB_ACTION_RETURN.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*     Trigger prompt chaining
      ev_function_response = lcl_llm_util->execute_prompt_chain( agent    = CONV zllm_function_name( lv_stackline )
                                                                 fc       = abap_true
                                                                 prompt   = cv_prompt )-response.
