FUNCTION z_llm_cod_soc_receipt.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"----------------------------------------------------------------------
  DATA : function_call_uuid TYPE guid16,
         ...
         business_function  TYPE zllm_buss_function_name VALUE 'EMAIL'.
...
* Set the global context of this FM.
      ...
      DATA hybris_order TYPE string.

      " Safely check for the parameter.
      READ TABLE it_function_parameters ASSIGNING FIELD-SYMBOL(<fs_param>)
                                       WITH KEY parameter_name = 'HYBRISORDER'.
      IF sy-subrc = 0.
        hybris_order = <fs_param>-parameter_value.
