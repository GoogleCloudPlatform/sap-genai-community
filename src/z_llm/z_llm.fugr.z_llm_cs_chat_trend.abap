...
*"----------------------------------------------------------------------
  DATA : function_call_uuid TYPE guid16,
         ...
         business_function  TYPE zllm_buss_function_name VALUE 'CHAT'.
...
data(lcl_report) = new zcl_llm_report_util( app_id = app_id
                                            business_function = business_function ).



          "ev_function_response = lcl_report->build_sentiment_report( start_date = start_date ).
