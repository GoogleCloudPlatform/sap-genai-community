FUNCTION z_llm_cod_soc_product_info.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
***       ------READ AGENT PARAMETERS EXTRACTED------
      ...
      IF articleid IS INITIAL.
        ...
        TRY.
            ...
            IF articledesc IS NOT INITIAL.
              ...
              DATA(response) = lcl_cs_hub_llm_util->llm_article_from_desc( query    = articledesc ).
...
*       Instantiate product info class
        DATA(lcl_llm_cs_hub_product_info) = NEW zcl_llm_cs_hub_product_info( ).

*       ---WEB DATA---
        response = lcl_llm_cs_hub_product_info->get_web_data( it_article_range = lt_article_range ).

        ev_function_response = ev_function_response && '\n\n\n' && response.
        CLEAR : response.

*       ---ARTICLE MEASUREMENTS---
        response = lcl_llm_cs_hub_product_info->get_article_measurements( it_article_range = lt_article_range ).
      ...
      ELSE.
        ...
        ev_function_response = |Apologies, I couldn't understand the product you are referring to|.
