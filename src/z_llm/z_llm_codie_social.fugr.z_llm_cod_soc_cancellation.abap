FUNCTION z_llm_cod_soc_cancellation.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
* Get the function module name. Due to the nature of ABAP call stack using system functions doesn't work.
...
* Setup performance trace.
  DATA(gcl_performance_trace) = NEW zcl_llm_performance_util( app_id            = app_id
                                                              business_function = business_function
                                                              uuid_key          = function_call_uuid ).

* Begin performance trace.
  gcl_performance_trace->start_perforamnce_trace( type = CONV string( lt_callstack[ 1 ]-blockname ) ).
...
* Order number found.
      DATA(lcl_llm) = NEW zcl_cs_hub_llm_util( hybris_order   = CONV bstnk( lv_hybris_order )
