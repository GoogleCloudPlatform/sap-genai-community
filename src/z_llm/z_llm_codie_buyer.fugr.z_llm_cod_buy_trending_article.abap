FUNCTION z_llm_cod_buy_trending_article.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
* Instantiate global classes.
  TRY.
    ...
    CATCH zcx_llm INTO DATA(lex_llm).
  ENDTRY.

* Call templated methods.
  DATA(lv_uuid)           = lcl_template_util->get_memory_id( memory_id = 'ZLLM_CODIE_UUID' ).
  DATA(app_id)            = lcl_template_util->get_memory_id( memory_id = 'CODIE' ). "'ZLLM_CODIE_APPID'
  DATA(business_function) = lcl_template_util->get_memory_id( memory_id = 'BUYER' ). "ZLLM_CODIE_BUSINESS_FUNCTION
...
* With the app id and business function now set, we should re-instantiate the template class.
      ...
      IF it_function_parameters IS NOT INITIAL.
        ...
        ls_article-param_value = CONV matnr18( ls_article-param_value ).
